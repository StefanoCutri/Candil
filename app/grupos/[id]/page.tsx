...
export default function GrupoDetallePage() {
  ...
  async function cargar() {
    ...
    if (!res.ok) { setError(data?.error ?? t('load_error')); setLoading(false); return }
  ...
  if (loading) return <Centro texto={t('loading_group')} />
  if (error || !d?.grupo) return <Centro texto={error || t('not_found')} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--ink)', fontWeight: 300 }}>
      <nav style={{ borderBottom: '0.5px solid var(--border)', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, background: 'var(--nav-bg)', backdropFilter: 'blur(12px)', zIndex: 50 }}>
        ...
        <Link href="/grupos" style={{ fontSize: 13, color: 'var(--ink-muted)', textDecoration: 'none' }}>← {t('title_short')}</Link>
      </nav>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 120px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 28 }}>
          <div>
            ...
            <button onClick={copiarCodigo} style={{ fontSize: 12, color: 'var(--amber)', background: 'var(--amber-dim)', border: '0.5px solid var(--border-mid)', borderRadius: 100, padding: '4px 12px', cursor: 'pointer', fontFamily: 'monospace', letterSpacing: '0.1em' }}>
              {copiado ? t('code_copied') : t('code_label', { code: d.grupo.codigo })}
            </button>
          </div>
          <button onClick={salir} style={{ fontSize: 12.5, color: 'rgba(200,90,90,0.8)', background: 'none', border: '0.5px solid rgba(200,90,90,0.25)', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>{t('leave_group')}</button>
        </div>

        {/* Compartir plan */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 32, flexWrap: 'wrap' }}>
          <select value={planSel} onChange={e => setPlanSel(e.target.value)} className="input" style={{ flex: 1, minWidth: 200 }}>
            <option value="">{t('share_select')}</option>
            {misPlanes.map(p => <option key={p.id} value={p.id}>{p.materia}</option>)}
          </select>
          <button onClick={compartir} disabled={!planSel} className="btn-primary" style={{ padding: '10px 18px', fontSize: 13, opacity: planSel ? 1 : 0.5 }}>{t('share_btn')}</button>
        </div>

        {/* Planes compartidos */}
        <h2 style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: 16 }}>{t('group_plans')}</h2>
        {d.planes.length === 0 ? (
          <p style={{ fontSize: 13, color: 'var(--ink-faint)', marginBottom: 36 }}>{t('no_shared')}</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 36 }}>
            {d.planes.map(p => (
              <a key={p.id} href={p.planes ? `/p/${p.planes.token_publico}` : '#'} target="_blank" rel="noreferrer"
                ...
                <span style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: '1rem', color: 'var(--ink)', flex: 1 }}>{p.planes?.examenes?.materia ?? t('plan_fallback')}</span>
                <span style={{ fontSize: 12, color: 'var(--ink-muted)' }}>{p.profiles?.nombre ?? t('someone')}</span>
                <span style={{ fontSize: 12, color: 'var(--amber)' }}>{t('view')}</span>
              </a>
            ))}
          </div>
        )}
        ...
        {/* Miembros */}
        <h2 style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: 16 }}>{t('members')} ({d.miembros.length})</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {d.miembros.map(m => {
            const nombre = m.profiles?.nombre ?? t('student')
            return (
              <div key={m.user_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 10, background: 'var(--surface)', border: '0.5px solid var(--border)' }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--ink)', flex: 1 }}>{nombre}{m.user_id === d.yo ? ` ${t('you')}` : ''}</span>
                {m.user_id === d.grupo!.owner_id && <span style={{ fontSize: 10, color: 'var(--amber)', letterSpacing: '0.06em' }}>{t('owner')}</span>}
              </div>
            )
          })}
        </div>
