        ...
        {bloqueado ? (
          <div style={{ textAlign: 'center', padding: '48px 24px', border: '0.5px dashed var(--border-mid)', borderRadius: 12 }}>
            <p style={{ fontSize: 14, color: 'var(--ink-soft)', marginBottom: 16, lineHeight: 1.6 }}>
              Los grupos de estudio son parte del plan <strong style={{ color: 'rgba(200,150,220,0.9)' }}>Plus</strong>.
            </p>
            <Link href="/precios" className="btn-primary">Ver Plus →</Link>
          </div>
        ) : loading ? (
          <p style={{ color: 'var(--ink-muted)', fontStyle: 'italic' }}>Cargando tus grupos…</p>
        ) : (
          <>
            {/* Crear / unirse */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12, marginBottom: 36 }}>
              <div style={{ padding: 18, borderRadius: 12, background: 'var(--surface)', border: '0.5px solid var(--border)' }}>
                <p style={{ fontSize: 13, color: 'var(--ink)', fontWeight: 500, marginBottom: 10 }}>Crear un grupo</p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <input className="input" value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej: Comisión 3 Fisio" style={{ flex: 1 }} />
                  <button onClick={() => accion('crear')} disabled={busy || !nombre.trim()} className="btn-primary" style={{ padding: '10px 16px', fontSize: 13, opacity: busy || !nombre.trim() ? 0.5 : 1 }}>Crear</button>
                </div>
              </div>
              <div style={{ padding: 18, borderRadius: 12, background: 'var(--surface)', border: '0.5px solid var(--border)' }}>
                <p style={{ fontSize: 13, color: 'var(--ink)', fontWeight: 500, marginBottom: 10 }}>Unirme con un código</p>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => accion('unirse')} disabled={busy || !codigo.trim()} className="btn-secondary" style={{ padding: '10px 16px', fontSize: 13, opacity: busy || !codigo.trim() ? 0.5 : 1 }}>Unirme</button>
                </div>
              </div>
            </div>
            ...
            {/* Mis grupos */}
            <h2 style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: 16 }}>Tus grupos</h2>
            {grupos.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--ink-faint)' }}>Todo tranquilo por acá. Creá un grupo o unite con un código.</p>
            ) : (
